...
class TestGeologicalPeriodController(BaseTestCase):
    """GeologicalPeriodController integration test stubs"""
    ...
    def test_geologicalperiods_id_get(self):
        ...
        response = self.client.open(
            '/v0.0.1/geologicalperiods/{id}'.format(id='id_example'),
            method='GET',
            headers=headers)
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
