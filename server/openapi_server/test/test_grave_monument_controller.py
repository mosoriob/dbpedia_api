...
class TestGraveMonumentController(BaseTestCase):
    """GraveMonumentController integration test stubs"""
    ...
    def test_gravemonuments_id_get(self):
        ...
        response = self.client.open(
            '/v0.0.1/gravemonuments/{id}'.format(id='id_example'),
            method='GET',
            headers=headers)
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
