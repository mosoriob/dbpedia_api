...
class TestChartsPlacementsController(BaseTestCase):
    """ChartsPlacementsController integration test stubs"""
    ...
    def test_chartsplacementss_id_get(self):
        ...
        response = self.client.open(
            '/v0.0.1/chartsplacementss/{id}'.format(id='id_example'),
            method='GET',
            headers=headers)
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
